...
export class Hex {
  ...
  static createHexTopGeometry(size: number, border: number): THREE.BufferGeometry {
    ...
    for (let i = 0; i < 6; i++) {
      const point = Formulas.hexVertex(i, innerRadius);
      const [u, v] = Formulas.planeCoordToTextureCoordinates(
        point,
        outerRadius,
      );
    ...
    for (let i = 0; i < 6; i++) {
      const point = Formulas.hexVertex(i, outerRadius);
      const [u, v] = Formulas.planeCoordToTextureCoordinates(
        point,
        outerRadius,
      );
  ...
  static createHexSidesGeometry(size: number): THREE.BufferGeometry {
    ...
    for (let i = 0; i < 6; i++) {
      outerPoints.push(Formulas.hexVertex(i, outerRadius));
    }
